...
path = './deeplearning/papago_project/model/'

# 사전에 학습된 모델을 이용한 예측 기능을 제공
def predict( src ):
    '''
        예측 담당 함수(머신러닝/딥러닝 기술으로 규칙을 완성)
        src : 입력, 예측 모델이 예측을 수행할 수 있는 형태로 제공, DataFrame, ( 개수, 65536 )
        return : 출력, 언어를 예측하여 국가코드 리턴, ndarray, ( 문자열정답, )
    '''
    # 1. 모델 로드 ( 엔트리포인트 기준에서 상대경로를 설정 or 절대 경로계산하여 설정 )
    model = joblib.load(path + 'ml.pkl')
...
def preprocessing( oriText=None ):
    if not oriText:
        return pd.DataFrame(np.load(path+'test_features.npy')) 
    else:
        '''
            한국어와 일본어등 고유한 문자를 쓰는 국가들은 굳이 학습하지 않아도 규칙성으로 해결 가능
            전체 문자 대비 영어의 비중이 80% 이상이면 알파벳으로 인지 설정
            입력 텍스트를 3개의 정규식으로 각각 처리후 각 무자별 위치의 카운트를 계산해서, 원문자 개수 대비
            80%이상이면 해당 문자로 분류하겠다(프로그래밍 규칙)
            이중 알파벳으로 판독되는 것은 모델로 예측 수행 => en, fr, tl, id 분류
            한국어, 일본어는 그대로 결론 도달함
        '''
        # 1. 텍스트 소문자 처리
...
if __name__=='__main__':
    ...
    print( predictMainStream('This should make it possible to check that the cross-validation score is in the same range as before.') )
    pass